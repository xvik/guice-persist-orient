 ...
 * <p>Default transaction manager implementation could be overridden by simply defining different implementation in guice context.</p>
 *
...
public interface TransactionManager extends UnitOfWork {
     ...
     * <p>If commit failed with error, error will be propagated. In case when two or more pools fail with errors, only last error
     * is propagated and other messages will just appear in log</p>
     */
    @Override
    void end();
    ...
    /**
     * If transaction has specific configuration for rollback exception classes, it's checked and if rollback shouldn't occur
     * commit is performed instead of rollback. Ends unit of work.
